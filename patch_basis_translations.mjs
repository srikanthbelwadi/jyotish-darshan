...
for (const lang of Object.keys(newTerms)) {
   const terms = newTerms[lang];
   const hook = new RegExp(`(${lang}:\\s*\\{[\\s\\S]*?)(?=(?:\\s*[a-z]+:\\s*\\{)|(?:\\}\\s*;\\s*$))`, 'g');
   
   src = src.replace(hook, (match, prefix) => {
       // Insert at end of prefix block before closing brace (if possible) or just append
       let appended = '';
       for (const k in terms) {
          appended += `\n    "${k}": "${terms[k]}",`;
       }
       return prefix.trim().replace(/,$/, '') + ',' + appended;
   });
}

// Write back
fs.writeFileSync(dictPath, src, 'utf8');
console.log("Patched basis terms into dynamicTranslations.js safely.");
