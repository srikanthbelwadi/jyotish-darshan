const mockKundalis = [
  {
    name: "Customer A - Aries Ascendant",
    kundaliData: {
      lagna: { rashi: 'Aries' },
...
async function runTest() {
  ...
  for (const J of mockKundalis) {
     console.log("\\n===========================================");
     console.log("Evaluating: " + J.name);
     
     console.log("\\n--- ORACLE: Today ---");
     const today1 = await fetchOracle('Today', J.kundaliData);
     const today2 = await fetchOracle('Today', J.kundaliData);
     console.log("Call 1 Basis:", today1.basis);
     console.log("Call 2 Basis:", today2.basis);

     console.log("\\n--- ORACLE: This Samvatsara ---");
     const samv1 = await fetchOracle('This Samvatsara', J.kundaliData);
     const samv2 = await fetchOracle('This Samvatsara', J.kundaliData);
     console.log("Call 1 Basis:", samv1.basis);
     console.log("Call 2 Basis:", samv2.basis);

     console.log("\\n--- PATHWAY: Wealth & Treasury ---");
     const p1 = await fetchPathway(J);
     const p2 = await fetchPathway(J);
     console.log("Call 1 Summary:", p1.summary);
     console.log("Call 2 Summary:", p2.summary);
     if(p1.options && p1.options[0]) {
       console.log("Call 1 Astrological Mechanics:", p1.options[0].paragraphs[0].content);
     }
     if(p2.options && p2.options[0]) {
       console.log("Call 2 Astrological Mechanics:", p2.options[0].paragraphs[0].content);
     }
  }
}
runTest();
