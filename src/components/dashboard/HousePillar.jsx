       ...
       <div className="desktop-sticky-panel" style={{ background: 'var(--bg-input)', padding: '40px', border: '2px solid var(--border-light)', boxShadow: 'inset 0 0 20px rgba(0,0,0,0.5)' }}>
         ...
         {opt.timeframe && (
           <div style={{ marginTop: '32px', textAlign: 'center', background: 'var(--bg-card)', padding: '16px', border: '1px solid var(--accent-gold)' }}>
             ...
             <div style={{ color: 'var(--text-main)', fontSize: '18px', fontFamily: 'var(--font-serif)' }}>{opt.timeframe}</div>
           </div>
         )}
       </div>
       <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
         <button onClick={() => setOpt(null)} style={{ background: 'var(--bg-card)', color: 'var(--accent-gold)', border: '1px solid #ffd700', padding: '8px 16px', cursor: 'pointer', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold', fontSize: '14px', fontFamily: 'var(--font-serif)', transition:'all 0.2s', textTransform:'uppercase' }} onMouseOver={e=>{e.currentTarget.style.background='var(--bg-input)'}} onMouseOut={e=>{e.currentTarget.style.background='var(--bg-card)'}}>
           {t('ig.backToPathways', '← BACK TO PATHWAYS')}
         </button>
         <div style={{ display: 'inline-block', background: 'var(--bg-card)', color: 'var(--accent-gold)', padding: '8px 16px', border: '1px solid #ffd700', fontSize: '14px', fontWeight: 'bold', fontFamily: 'var(--font-serif)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '20px' }}>{t('Subject:')} {t(opt.label)}</div>
         <h3 style={{ color: 'var(--text-main)', fontSize: '30px', marginTop: 0, marginBottom: '32px', lineHeight: 1.3, fontFamily: 'var(--font-serif)', textShadow: '0 2px 4px var(--bg-surface)' }}>{t(data.title)}</h3>
         ...
         <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%' }}>
           {opt.paragraphs && opt.paragraphs.map((para, idx) => (
             <div key={idx} style={{ background: 'var(--bg-input)', padding: '24px', borderTop: '4px solid #ffd700', boxShadow: '0 4px 15px rgba(0,0,0,0.2)', textAlign: 'center' }}>
               {para.subheading && (
                 <div style={{ color: 'var(--text-main)', fontSize: '16px', fontWeight: 'bold', fontFamily: 'var(--font-serif)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                   <span style={{ fontSize: '20px', color: 'var(--accent-gold)' }}>✧</span> {para.subheading}
                 </div>
               )}
               <p style={{ color: 'var(--text-main)', fontSize: '18px', lineHeight: 1.7, margin: '0 auto', fontFamily: 'var(--font-serif)', maxWidth: '800px' }}>{para.content}</p>
             </div>
           ))}
           ...
           {opt.prediction && !opt.paragraphs && (
             <div style={{ background: 'var(--bg-input)', padding: '24px', borderTop: '4px solid #ffd700', boxShadow: '0 4px 15px rgba(0,0,0,0.5)', textAlign: 'center' }}>
               <div style={{ color: 'var(--text-main)', fontSize: '14px', fontWeight: 'bold', fontFamily: 'var(--font-serif)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                 <span style={{ fontSize: '18px' }}>👁️</span> {t('Prophetic Unfoldment')}
               </div>
               <p style={{ color: 'var(--text-main)', fontSize: '18px', lineHeight: 1.7, margin: '0 auto', fontFamily: 'var(--font-serif)', maxWidth: '800px' }}>{opt.prediction}</p>
             </div>
           )}
         </div>
