...
for lang, header_str in headers.items():
    ...
    if content == old_content:
        # Try without newline
        pattern2 = rf"({lang}:{{.*?tagline:'.*?',)"
        content = re.sub(pattern2, repl, content, count=1, flags=re.DOTALL)
        if content == old_content:
            print(f"Failed to match for lang: {lang}")

with open('src/App.jsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Patching headers complete.")
