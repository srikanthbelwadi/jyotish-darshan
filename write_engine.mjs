...
export function calculateMatch(k1, k2) {
  ...
  let dashaWarning = "Dasha periods are reasonably balanced.";
  if (['rahu','ketu','saturn'].includes(str1.dashaPlanet) && ['rahu','ketu','saturn'].includes(str2.dashaPlanet)) {
      dashaWarning = "CAUTION: Both partners are running intense karmic Mahadashas (Rahu/Ketu/Saturn). Conscious patience is essential.";
  }

  const structuralSynthesis = \`The 7th House Lords are \${str1.h7Lord} (\${p1.name}) and \${str2.h7Lord} (\${p2.name}). \${str1.venusStrong && str2.venusStrong ? 'Venus is strongly placed offering marital comforts.' : 'Venus requires strengthening for full marital bliss.'} \${dashaWarning}\`;
  ...
  return {
    ...
    mangalDosha: {
        manglikStatus,
        manglikKey,
        p1Manglik: m1.isManglik,
        p2Manglik: m2.isManglik,
        mutualCancellation
    },
    structural: {
...
`;
fs.writeFileSync('src/engine/matchmaking.js', content);
console.log("Written match engine");
